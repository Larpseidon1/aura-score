...
export async function GET(request: NextRequest) {
  try {
    ...
    // Enrich projects with external API data
    const enrichedProjects = await enrichAllProjects(BASE_PROJECTS);

    // Calculate summary statistics
    const infrastructureProjects = enrichedProjects.filter(p => p.category === 'L1' || p.category === 'L2');
    const applicationProjects = enrichedProjects.filter(p => p.category === 'Application' || p.category === 'Stablecoins');

    const totalInfrastructure = {
      count: infrastructureProjects.length,
      totalRaised: infrastructureProjects.reduce((sum, p) => sum + p.amountRaised, 0),
      totalRevenue: infrastructureProjects.reduce((sum, p) => sum + (p.ecosystemRevenue || p.revenue || 0), 0),
      avgValuation: infrastructureProjects.reduce((sum, p) => sum + (p.fdv || p.marketCap || 0), 0) / infrastructureProjects.length
    };

    const totalApplications = {
      count: applicationProjects.length,
      totalRaised: applicationProjects.reduce((sum, p) => sum + p.amountRaised, 0),
      totalRevenue: applicationProjects.reduce((sum, p) => sum + (p.revenue || 0), 0),
      avgValuation: applicationProjects.reduce((sum, p) => sum + (p.fdv || p.marketCap || 0), 0) / applicationProjects.length
    };
