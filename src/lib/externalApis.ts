...
async function fetchAppFees(chainSlug: string): Promise<number | null> {
  try {
    console.log(`🔄 Fetching app fees for ${chainSlug}...`);
    
    // Special handling for Hyperliquid - use chain app fees from the L1 endpoint
    if (chainSlug === 'hyperliquid') {
      const response = await rateLimitedFetch(`${DEFILLAMA_BASE_URL}/overview/fees/hyperliquid-l1`);
      
      if (!response.ok) {
        console.log(`❌ Failed to fetch DeFiLlama chain app fees for hyperliquid-l1: ${response.status}`);
        return null;
      }
      
      const data = await response.json();
      
      // For Hyperliquid L1, prefer the already-calculated yearly total if available
      let annualizedAppFees: number;
      let dataSource: string;
      
      if (data.total1y) {
        annualizedAppFees = data.total1y;  // Use the yearly total directly
        dataSource = 'yearly total';
      } else if (data.total30d) {
        annualizedAppFees = data.total30d * 12;  // 30-day total × 12 months
        dataSource = '30d total × 12';
      } else if (data.total7d) {
        annualizedAppFees = data.total7d * 52;  // 7-day total × 52 weeks
        dataSource = '7d total × 52';
      } else {
        annualizedAppFees = (data.total24h || 0) * 365;  // 24h × 365 days
        dataSource = '24h × 365';
      }
      
      console.log(`✅ hyperliquid: $${annualizedAppFees.toLocaleString()} annual app fees (${dataSource} from L1 chain)`);
      return annualizedAppFees;
    }
    
    // For all other chains, use the standard app fees API
    const response = await rateLimitedFetch(`${DEFILLAMA_BASE_URL}/overview/fees/${chainSlug}`);
    
    if (!response.ok) {
      console.log(`❌ Failed to fetch DeFiLlama app fees for ${chainSlug}: ${response.status}`);
      return null;
    }
    
    const data = await response.json();
    
    // Consistent 30d × 12 methodology, fallback to 7d × 52, then 24h × 365
    let annualizedAppFees: number;
    let dataSource: string;
    
    if (data.total30d) {
      annualizedAppFees = data.total30d * 12;  // 30-day total × 12 months (consistent)
      dataSource = '30d total × 12';
    } else if (data.total7d) {
      annualizedAppFees = data.total7d * 52;  // 7-day total × 52 weeks (fallback)
      dataSource = '7d total × 52';
    } else {
      annualizedAppFees = (data.total24h || 0) * 365;  // 24h × 365 days (last resort)
      dataSource = '24h × 365';
    }
    
    console.log(`✅ ${chainSlug}: $${annualizedAppFees.toLocaleString()} annual app fees (${dataSource})`);
    return annualizedAppFees;
    
  } catch (error) {
    console.log(`❌ Error fetching app fees for ${chainSlug}:`, error);
    return null;
  }
}
